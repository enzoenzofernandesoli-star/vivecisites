  ...
  return <main ref={depthRoot} className={styles.site}>
    ...
    <section className={styles.technology} id="tecnologia">
      <div className={styles.technologyHeading}>
        <div><span>Tecnologia</span><TextReveal as="h2" plano>Uma base sólida. Infinitas possibilidades.</TextReveal></div>
      ...
      <div className={styles.technologyStack}>
        {technologyLayers.map((layer, index) => <Reveal as="article" index={index} plano className={styles.technologyLayer} key={layer.title}>
          ...
          <Stagger className={styles.technologyItems} plano intervalo={0.05} deslocamento={12}>{layer.technologies.map((technology) => <div className={styles.technologyItem} key={technology}><TechIcon name={technology}/><span>{technology}</span></div>)}</Stagger>
        </Reveal>)}
      </div>
