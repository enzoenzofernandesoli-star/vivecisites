...
with sync_playwright() as p:
    ...
    page.wait_for_selector('canvas[data-robot-canvas="true"]',state="attached")
    robot=page.locator('canvas[data-robot-canvas="true"]')
    assert robot.count()==1
    assert page.locator('canvas[data-spatial-scene="true"]').count()==0
    assert robot.evaluate("el=>el.width>1200 && el.height>500")
    assert robot.evaluate("el=>Boolean(el.getContext('webgl2')||el.getContext('webgl'))")
    assert page.locator('[class*="heroSpatialScene"][data-pronto="true"]').count()==1
    assert (Path(__file__).resolve().parents[1]/"public/models/robot-expressive.glb").stat().st_size==463988
    assert "CC0 1.0" in (Path(__file__).resolve().parents[1]/"public/models/ATTRIBUTION.md").read_text(encoding="utf-8")
    ...
    results.append({"desktop_3d":True,"scroll_robot":True,"old_orbits_removed":True,"reduced_motion_cleanup":True,"errors":errors})
    page.close()
